#!/usr/bin/env node
// === 300일 생존 시뮬레이션 + 퀘스트 추적 (이지수 · 의사) — PATCHED ===
// 실제 코드의 동료/능력 로직 반영 버전
// node sim_jisu_patched.mjs
...
const DOCTOR_QUESTS = [
  // Phase 1: 생존 (Day 1–14)
  { id:'mq_doctor_01', title:'삼성병원 생존자',    icon:'🏥', dayTrigger:1,  prerequisite:null,           objective:{type:'collect_item_type',itemType:'food',count:3},          deadlineDays:10,       reward:{morale:5} },
  { id:'mq_doctor_02', title:'처치실 방벽',        icon:'🏗️', dayTrigger:2,  prerequisite:'mq_doctor_01', objective:{type:'craft_structure',count:1},                             deadlineDays:12,       reward:{morale:5, bandage:2} },
  { id:'mq_doctor_03', title:'응급 처치 준비',     icon:'🩹', dayTrigger:4,  prerequisite:'mq_doctor_02', objective:{type:'collect_item',definitionId:'bandage',count:3},         deadlineDays:14,       reward:{morale:5} },
  { id:'mq_doctor_04', title:'환자 수액',          icon:'💧', dayTrigger:6,  prerequisite:'mq_doctor_03', objective:{type:'collect_clean_water',count:3},                         deadlineDays:16,       reward:{morale:5} },
  { id:'mq_doctor_05', title:'임시 의무실',        icon:'⚕️', dayTrigger:8,  prerequisite:'mq_doctor_04', objective:{type:'collect_item_type',itemType:'medical',count:3},        deadlineDays:18,       reward:{morale:5} },
  { id:'mq_doctor_06', title:'약초 채취',          icon:'🌿', dayTrigger:10, prerequisite:'mq_doctor_05', objective:{type:'collect_item',definitionId:'herb',count:2},            deadlineDays:20,       reward:{morale:5} },
  { id:'mq_doctor_07', title:'첫 번째 치료약',     icon:'💊', dayTrigger:13, prerequisite:'mq_doctor_06', objective:{type:'craft_medical',count:1},                               deadlineDays:23,       reward:{morale:8, bandage:1} },
  // Phase 2: 목적 — 세브란스 원정 (Day 15–40)
  { id:'mq_doctor_08', title:'원정 의료 물자',     icon:'🎒', dayTrigger:15, prerequisite:'mq_doctor_07', objective:{type:'collect_item_type',itemType:'medical',count:5},        deadlineDays:30,       reward:{morale:8} },
  { id:'mq_doctor_09', title:'홍대 약국 경유',     icon:'🗺️', dayTrigger:18, prerequisite:'mq_doctor_08', objective:{type:'visit_district',districtId:'mapo'},                    deadlineDays:35,       reward:{morale:10, antiseptic:1} },
  { id:'mq_doctor_10', title:'야전 소독',          icon:'🧴', dayTrigger:21, prerequisite:'mq_doctor_09', objective:{type:'collect_item',definitionId:'antiseptic',count:2},      deadlineDays:38,       reward:{morale:5} },
  { id:'mq_doctor_11', title:'이동 수분',          icon:'💦', dayTrigger:24, prerequisite:'mq_doctor_10', objective:{type:'collect_clean_water',count:5},                         deadlineDays:40,       reward:{morale:5} },
  { id:'mq_doctor_12', title:'세브란스 연구소',    icon:'🔬', dayTrigger:28, prerequisite:'mq_doctor_11', objective:{type:'visit_district',districtId:'seodaemun'},               deadlineDays:55,       reward:{morale:15} },
  { id:'mq_doctor_13', title:'데이터 분석 시약',   icon:'📊', dayTrigger:32, prerequisite:'mq_doctor_12', objective:{type:'collect_item',definitionId:'first_aid_kit',count:1},  deadlineDays:60,       reward:{morale:8} },
  { id:'mq_doctor_14', title:'연구 기간',          icon:'⏳', dayTrigger:35, prerequisite:'mq_doctor_13', objective:{type:'survive_days',count:35},                               deadlineDays:Infinity, reward:{morale:10} },
  { id:'mq_doctor_15', title:'연구 기간 비축',     icon:'🌾', dayTrigger:38, prerequisite:'mq_doctor_14', objective:{type:'collect_item_type',itemType:'food',count:5},           deadlineDays:60,       reward:{morale:5} },
  // Phase 3: 위기 — 프로토콜 개발 (Day 40–68)
  { id:'mq_doctor_16', title:'치료제 초안',        icon:'⚗️', dayTrigger:40, prerequisite:'mq_doctor_15', objective:{type:'craft_medical',count:2},                               deadlineDays:65,       reward:{morale:10} },
  { id:'mq_doctor_17', title:'천연 항바이러스',    icon:'🌿', dayTrigger:43, prerequisite:'mq_doctor_16', objective:{type:'collect_item',definitionId:'herb',count:5},            deadlineDays:68,       reward:{morale:8} },
  { id:'mq_doctor_18', title:'대규모 치료',        icon:'🏥', dayTrigger:46, prerequisite:'mq_doctor_17', objective:{type:'collect_item_type',itemType:'medical',count:8},        deadlineDays:72,       reward:{morale:10} },
  { id:'mq_doctor_19', title:'치료소 설치',        icon:'⛺', dayTrigger:49, prerequisite:'mq_doctor_18', objective:{type:'craft_structure',count:2},                             deadlineDays:76,       reward:{morale:10} },
  { id:'mq_doctor_20', title:'동작구 치료 활동',   icon:'🏘️', dayTrigger:52, prerequisite:'mq_doctor_19', objective:{type:'visit_district',districtId:'dongjak'},                deadlineDays:80,       reward:{morale:12} },
  { id:'mq_doctor_21', title:'부상자 치료',        icon:'🩹', dayTrigger:55, prerequisite:'mq_doctor_20', objective:{type:'collect_item',definitionId:'bandage',count:8},         deadlineDays:83,       reward:{morale:8} },
  { id:'mq_doctor_22', title:'최종 프로토콜',      icon:'📋', dayTrigger:58, prerequisite:'mq_doctor_21', objective:{type:'craft_medical',count:3},                               deadlineDays:88,       reward:{morale:15} },
  { id:'mq_doctor_23', title:'60일 기록',          icon:'📖', dayTrigger:62, prerequisite:'mq_doctor_22', objective:{type:'survive_days',count:60},                               deadlineDays:Infinity, reward:{morale:12} },
  // Phase 4: 해결 — 삼성병원 의료 허브 (Day 65–100)
  { id:'mq_doctor_24', title:'삼성병원 귀환',      icon:'🏠', dayTrigger:65, prerequisite:'mq_doctor_23', objective:{type:'visit_district',districtId:'gangnam'},                 deadlineDays:90,       reward:{morale:15} },
  { id:'mq_doctor_25', title:'허브 물자 비축',     icon:'📦', dayTrigger:68, prerequisite:'mq_doctor_24', objective:{type:'collect_item',definitionId:'first_aid_kit',count:5},  deadlineDays:92,       reward:{morale:10} },
  { id:'mq_doctor_26', title:'의료 시설 확충',     icon:'🏗️', dayTrigger:71, prerequisite:'mq_doctor_25', objective:{type:'craft_structure',count:3},                             deadlineDays:95,       reward:{morale:10} },
  { id:'mq_doctor_27', title:'대규모 배포 물자',   icon:'🤝', dayTrigger:74, prerequisite:'mq_doctor_26', objective:{type:'collect_item_type',itemType:'medical',count:10},       deadlineDays:97,       reward:{morale:12} },
  { id:'mq_doctor_28', title:'치료약 대량 생산',   icon:'⚗️', dayTrigger:78, prerequisite:'mq_doctor_27', objective:{type:'craft_medical',count:5},                               deadlineDays:Infinity, reward:{morale:15} },
  { id:'mq_doctor_29', title:'의료 허브 완성',     icon:'🏆', dayTrigger:83, prerequisite:'mq_doctor_28', objective:{type:'collect_clean_water',count:10},                        deadlineDays:Infinity, reward:{morale:20} },
  { id:'mq_doctor_30', title:'100일의 기록',       icon:'📝', dayTrigger:88, prerequisite:'mq_doctor_29', objective:{type:'survive_days',count:100},                              deadlineDays:Infinity, reward:{morale:25} },
];
...
function createGameState() {
  return {
    ...
    inv: {
      // 캐릭터 정의(characters.js)와 일치: 붕대×4, 소독약, 메스, 통조림×2, 에너지바×2
      bandage: 4, antiseptic: 1, painkiller: 1, gauze: 0,
      canned_food: 2, water_bottle: 2, energy_bar: 2, rice: 0,
      ...
      knife: 0, crowbar: 0,
      combat_scalpel: 1,  // 의사 시작 무기 (dmg [8,14])
      stethoscope: 1,     // 감염 진단 도구 (패시브)
      herb: 0,
    },

    // ─── 이지수 고유 능력 ──────────────────────────────
    abilities: {
      healBonus:      1.5,   // trauma_expert: 의료 효과 +50%
      infectionRate:  0.75,  // clinical_knowledge: 감염 진행 -25%
      bandageHpBonus: 5,     // emergency_care: 붕대 사용 시 +5 HP
    },

    // ─── 동료 (실제 게임은 Day 1부터 간호사 동반) ─────
    companions: ['npc_nurse'],
    nurseHp: 100, nurseMaxHp: 100,

    // 박상훈 하사: 부상 상태로 시작 (붕대 3개 치료 후 영입 가능)
    woundedSoldier: {
      woundLevel: 3,   // 붕대 1개 = 1단계 치료
      recruited:  false,
      hp: 100, maxHp: 100,
    },
...
function checkQuestProgress(gs) {
  for (let i = gs.quests.active.length - 1; i >= 0; i--) {
    ...
    if (q.progress >= target) {
      ...
      if (qDef.reward.antiseptic) gs.inv.antiseptic += qDef.reward.antiseptic;
...
function advanceTP(gs) {
  ...
  if (gs.tpInDay >= TP_PER_DAY) {
    ...
    // P1: 후반 절망 대응 — 동료 대화로 사기 회복
    // 조건: day >= 30, morale < 30, 동료 1명 이상 → nurse +4, soldier +3
    if (gs.day >= 30 && gs.morale < 30) {
      const hasNurse = (gs.companions ?? []).includes('npc_nurse');
      const hasSoldier = gs.woundedSoldier?.recruited === true;
      if (hasNurse || hasSoldier) {
        const restore = hasNurse ? 4 : 3;
        gs.morale = clamp(gs.morale + restore, 0, gs.maxMorale);
        gs.lateMoraleRestoreCount = (gs.lateMoraleRestoreCount ?? 0) + 1;
      }
    }
  }
...
function checkEnvironmentDisease(gs) {
  ...
  const infRate = gs.abilities?.infectionRate ?? 1.0;  // clinical_knowledge × 0.75
  const mult = sm.infectionMult * infRate;
...
function simulateCombat(gs, enemies) {
  gs.combatCount++;
  const hasWeapon = gs.inv.combat_scalpel > 0 || gs.inv.knife > 0 || gs.inv.crowbar > 0;
  ...
  // 무기별 데미지 (실제 items_combat.js 기반)
  let pDmg;
  if      (gs.inv.crowbar > 0)         pDmg = [10, 18];
  else if (gs.inv.combat_scalpel > 0)  pDmg = [8, 14];  // 의사 전용 메스
  else if (gs.inv.knife > 0)           pDmg = [8, 15];
  else                                 pDmg = [3, 7];
  const pAcc = 0.70;

  // 동료 전투 보너스: 박상훈 영입 시 × 1.3 (getCompanionCombatBonus 반영)
  const companionDmgMult = gs.woundedSoldier?.recruited ? 1.3 : 1.0;
  const nurseActive = gs.companions.includes('npc_nurse');
  const soldierActive = gs.woundedSoldier?.recruited;
  ...
  while (aliveEnemies.length > 0 && gs.alive && round < maxRounds) {
    ...
    if (Math.random() < pAcc) {
      ...
      dmg = Math.floor(dmg * companionDmgMult);
      target.hp -= dmg;
      if (target.hp <= 0) {
        aliveEnemies.shift();
        gs.totalKills++;
    ...
    for (const enemy of aliveEnemies) {
      if (Math.random() < enemy.acc) {
        ...
        // ─── 간호사: 피해 -10%, 15% 확률 대리타격 ───
        if (nurseActive && gs.companions.includes('npc_nurse')) {
          dmg = Math.max(1, Math.floor(dmg * 0.90));
          if (Math.random() < 0.15) {
            gs.nurseHp -= dmg;
            if (gs.nurseHp <= 0) {
              gs.companions = gs.companions.filter(c => c !== 'npc_nurse');
            }
            continue;  // 플레이어 피해 스킵
          }
        }

        // ─── 박상훈(영입 후): 피해 -15%, 20% 확률 대리타격 ───
        if (soldierActive && gs.woundedSoldier?.recruited) {
          dmg = Math.max(1, Math.floor(dmg * 0.85));
          if (Math.random() < 0.20) {
            gs.woundedSoldier.hp -= dmg;
            if (gs.woundedSoldier.hp <= 0) {
              gs.woundedSoldier.recruited = false;  // 쓰러짐
            }
            continue;
          }
        }
        ...
        gs.hp -= dmg;
        // 감염: clinical_knowledge × 0.75 적용
        const infRate = gs.abilities?.infectionRate ?? 1.0;
        if (Math.random() < 0.20 * infRate) {
  ...
  // 전투 승리 후 간호사가 동료 일부 회복
  if (aliveEnemies.length === 0) {
    if (nurseActive && gs.companions.includes('npc_nurse') && gs.nurseHp < gs.nurseMaxHp) {
      gs.nurseHp = clamp(gs.nurseHp + 15, 0, gs.nurseMaxHp);
    }
    if (gs.woundedSoldier?.recruited && gs.woundedSoldier.hp < gs.woundedSoldier.maxHp) {
      gs.woundedSoldier.hp = clamp(gs.woundedSoldier.hp + 10, 0, gs.woundedSoldier.maxHp);
    }
...
function consumeMedical(gs) {
  // ── 회복 배율: trauma_expert (1.5) × 간호사 healBonus (1.5) = 최대 2.25 ──
  const baseMult = gs.abilities?.healBonus ?? 1.0;
  const nurseMult = gs.companions?.includes('npc_nurse') ? 1.5 : 1.0;
  const healMult  = baseMult * nurseMult;
  const bandageBonus = gs.abilities?.bandageHpBonus ?? 0;
  ...
  if (hasLaceration && gs.inv.first_aid_kit > 0) {
    ...
    gs.hp = clamp(gs.hp + 75 * healMult, 0, gs.maxHp);
  ...
  // 출혈 → 붕대(+ emergency_care 보너스) 또는 거즈
  if (hasBleeding && gs.inv.bandage > 0) {
    gs.inv.bandage--;
    gs.hp = clamp(gs.hp + 28 * healMult + bandageBonus, 0, gs.maxHp);
    gs.diseases = gs.diseases.filter(d => d.id !== 'bleeding');
    return true;
  }
  if (hasBleeding && gs.inv.gauze > 0) {
    gs.inv.gauze--;
    gs.hp = clamp(gs.hp + 15 * healMult, 0, gs.maxHp);
    gs.diseases = gs.diseases.filter(d => d.id !== 'bleeding');
    return true;
  }
...
function aiTurn(gs) {
  ...
  // ── 박상훈 하사 치료 (붕대 여유 있을 때만) ──
  if (gs.woundedSoldier && !gs.woundedSoldier.recruited && gs.woundedSoldier.woundLevel > 0) {
    // 본인 응급용 붕대 2개는 남겨둠
    if (gs.inv.bandage >= 3) {
      gs.inv.bandage--;
      gs.woundedSoldier.woundLevel--;
      if (gs.woundedSoldier.woundLevel <= 0) {
        gs.woundedSoldier.recruited = true;
        gs.morale = clamp(gs.morale + 15, 0, gs.maxMorale);  // 완치 보상
      }
      advanceTP(gs);
      if (!gs.alive) return;
    }
  ...
  const needsMedCraft = gs.quests.active.some(q => {
    const qDef = DOCTOR_QUESTS.find(d => d.id === q.id);
    return qDef?.objective?.type === 'craft_medical' && gs.totalMedicalCrafted < (qDef.objective.count ?? 1);
  });
  if (needsMedCraft && gs.inv.herb >= 1) {
    tryCraftMedical(gs);
    if (!gs.alive) return;
  }
...
function runOneGame(runId) {
  ...
  return {
    ...
    lateMoraleRestoreCount: gs.lateMoraleRestoreCount ?? 0,
    events,
    quests: {
...
// ── 후반 사기 회복 이벤트 통계 (P1) ──────────────────────
const lateMoraleEvents = results.map(r => r.lateMoraleRestoreCount ?? 0);
const totalLateMoraleEvents = lateMoraleEvents.reduce((s, v) => s + v, 0);
const runsWithLateMoraleEvent = lateMoraleEvents.filter(v => v > 0).length;
lines.push('── P1: 후반 사기 회복 이벤트 ──');
lines.push(`  이벤트 발생 런수: ${runsWithLateMoraleEvent}/${NUM_RUNS} (${((runsWithLateMoraleEvent/NUM_RUNS)*100).toFixed(1)}%)`);
lines.push(`  총 발화 횟수: ${totalLateMoraleEvents} (평균 ${(totalLateMoraleEvents/NUM_RUNS).toFixed(2)}회/런)`);
lines.push('');
...
const outputPath = 'sim_jisu_patched_result.txt';
