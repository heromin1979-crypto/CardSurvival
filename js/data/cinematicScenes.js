...
const CINEMATIC_SCENES = {
  // ── 1차 분기 연출 (6개) — D21, 여섯 직업 전원이 통과하는 지점 ──────
  // 시네마틱 42개 중 분기·엔딩용이 4개뿐이었다. 모든 플레이어가 반드시
  // 지나는 자리에 연출이 없어 선택의 무게가 전달되지 않았다.
  // 색상은 엔딩 팔레트의 직업별 색상(hue)을 따른다.

  cin_branch_doctor: {
    id: 'cin_branch_doctor',
    gradient: 'linear-gradient(160deg,#03181c 0%,#06333b 60%,#010e11 100%)',
    title: '베인 손을 펴다',
    subtitle: '홍대 — 세 갈래',
    lines: [
      '유리 파편에 손을 베었다. 남의 약을 빌릴 수는 없었다.',
      '제 손으로 만든 약 한 개를 제 상처에 발랐다.',
      '약국 뒷창고의 메모, 용산에서 잡힌 무전, 그리고 아무도 없는 보라매.',
      '셋이 동시에 손짓했다. 하나를 잡으면 둘은 닫힌다.',
    ],
    displayMs: 0,
  },

  cin_branch_soldier: {
    id: 'cin_branch_soldier',
    gradient: 'linear-gradient(160deg,#14180a 0%,#282f12 60%,#0b0d05 100%)',
    title: '두 개의 주파수',
    subtitle: '용산 — 선택의 밤',
    lines: [
      '무전기에 신호가 둘 잡혔다. 하나는 사람이고 하나는 방송국이다.',
      '박영철의 목소리. "구조 인력이 부족합니다."',
      '여의도 KBS의 반송파는 아직 살아 있다. 아무도 말하지 않을 뿐.',
      '손이 갈 수 있는 주파수는 하나다. 박상현, 어느 쪽이냐.',
    ],
    displayMs: 0,
  },

  cin_branch_engineer: {
    id: 'cin_branch_engineer',
    gradient: 'linear-gradient(160deg,#08101f 0%,#122139 60%,#040910 100%)',
    title: '연료 계산',
    subtitle: '성수동 — 두 갈래',
    lines: [
      '아버지의 공장에 돌아왔다. 공구는 걸이마다 제자리에 있었다.',
      '경유 잔량 68리터. 나가는 데 쓰면 사흘, 돌리는 데 쓰면 보름.',
      '설계도는 나가라고 그린 것이었다. 도시는 남으라고 부른다.',
      '정대한은 계산을 멈췄다. 이건 계산으로 나오지 않는다.',
    ],
    displayMs: 0,
  },
  ...
  cin_branch_chef: {
    id: 'cin_branch_chef',
    gradient: 'linear-gradient(160deg,#1d1104 0%,#38210a 60%,#100902 100%)',
    title: '불을 어디에 둘 것인가',
    subtitle: '남대문 — 두 갈래',
    lines: [
      '하루 마흔 그릇. 냄비 하나로 낼 수 있는 한계였다.',
      '넓히면 묽어지고 좁히면 깊어진다. 주방의 오래된 법칙이다.',
      '강 건너로 루트를 뻗을 것인가, 용산의 한 접시로 갈 것인가.',
      '윤재혁은 칼을 내려놓고 앞치마에 손을 닦았다.',
    ],
    displayMs: 0,
  },

  cin_branch_firefighter: {
    id: 'cin_branch_firefighter',
    gradient: 'linear-gradient(160deg,#200604 0%,#3d0d08 60%,#120302 100%)',
    title: '빨간 현관문까지 8킬로',
    subtitle: '서대문 — 두 갈래',
    lines: [
      '서대문. 여기서 불광동까지 8킬로. 하루면 닿는다.',
      '등 뒤에는 갈 곳 없는 사람들이 서른 남짓 서 있었다.',
      '진입은 늘 한 곳부터다. 두 곳을 동시에 열 수는 없다.',
      '이재훈이라면 어느 쪽 문을 먼저 열었을까.',
    ],
    displayMs: 0,
  },

  cin_branch_homeless: {
    id: 'cin_branch_homeless',
    gradient: 'linear-gradient(160deg,#061409 0%,#0d2a15 60%,#030b05 100%)',
    title: '두 번째 장부',
    subtitle: '잠실대교 남단 — 두 갈래',
    lines: [
      '강을 건넜다. 남은 것은 사람 열둘, 통조림 스물, 신용 0.',
      '의사가 손을 내밀었다. 지분을 나누자는 뜻이다.',
      '타워는 비어 있다. 혼자 세우면 전부 내 것이고 전부 내 책임이다.',
      '첫 번째 회사는 남의 보증으로 무너졌다. 최형식은 오래 생각했다.',
    ],
    displayMs: 0,
  },
