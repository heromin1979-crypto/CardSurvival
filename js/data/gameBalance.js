...
const BALANCE = {
  ...
  // ── 물 공급 구조물 ───────────────────────────────────
  // subtype:'water' + onTick.hydration을 가진 구조물이 대상.
  // 'rain_catch' 태그가 있으면 빗물을 받는 구조라 날씨 배율을 타고 산성비에 멈춘다.
  // 태그가 없으면(급수탑·배관·재활용기) 날씨와 무관하게 선언값을 그대로 공급한다.
  waterSupply: {
    // 합산 상한(/TP). 갈증 소모(hydrationDecayPerTP=1.0)와 같은 값이라 최대치에서도
    // 수분이 차오르지는 않는다. 비 오는 날 상한에 닿아 갈증이 멈추는 것이 인프라를
    // 완성한 보상이고, 상시 급수 3종을 다 지어도 합이 0.65라 평상시엔 물통이 필요하다.
    maxHydrationPerTP:    1.0,
    rainCatchDecayPerTP:  0.5,   // 빗물 수집 계열 내구도 마모 (산성비 중에는 마모 없음)
    supplyDecayPerTP:     0.15,  // 상시 급수 계열 내구도 마모 — 상시 가동이라 더 느리게 닳는다
  },
  ...
  medicalStation: {
    ...
    infectionResistCap:   0.8,    // 방역 시설 중첩 시 감염 증가분 감소 상한
  },
