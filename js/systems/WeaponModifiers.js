...
// 장전한 화살이 실어주는 보정. 부착물과 달리 탄창을 비우면 사라진다.
function loadedAmmoEffect(instance) {
  const id = instance?.loadedAmmoId;
  if (typeof id !== 'string') return null;
  return GameData.items?.[id]?.ammoEffect ?? null;
}

export function getWeaponModifiers(instance) {
  const ammo = loadedAmmoEffect(instance);
  return {
    damageBonus:    signed(instance?.damageBonus) + signed(ammo?.damageBonus),
    ...
    // 장전 화살이 부착물 상태이상을 덮는다 — 소모품이 영구 개조보다 우선한다
    statusInflict:  ammo?.statusInflict ?? instance?._statusInflict ?? null,
    multiTarget:    nonneg(ammo?.multiTarget),
    noiseOverride:  Number.isFinite(ammo?.noiseOnUse) ? ammo.noiseOnUse : null,
  };
}
