...
const BAL_HELP = {
  ...
  poisonCoating: '무기 독 도포 상한 — 독 피해는 적 방어를 우회하므로 상한을 둔다.',
  perApply: '독 1회 도포로 오르는 독 피해.',
  rawMax: '독버섯을 무기에 직접 바를 때의 독 피해 상한.',
  extractedMax: '절구로 추출한 독을 바를 때의 독 피해 상한(직접 도포보다 높다).',
  ...
  // 랭크 전투 — 속도/이니셔티브/회피
  defaultPlayerSpeed: '플레이어 기본 속도(라운드 이니셔티브 기준값).',
  defaultCompanionSpeed: '동료 기본 속도.',
  defaultEnemySpeed: '적 기본 속도(enemy.speed 미지정 시).',
  initiativeRollMax: '라운드 시작 이니셔티브 랜덤 굴림 최대값(속도에 가산).',
  defaultPlayerDodge: '플레이어 기본 회피 확률(토큰과 별개, 소비되지 않음).',
  defaultCompanionDodge: '동료 기본 회피 확률.',
  // 죽음의 문턱
  baseResist: '죽음의 문턱 최초 저항 확률(피격 시 사망 회피).',
  resistLossPerCheck: '저항 판정마다 감소하는 저항 확률.',
  minimumResist: '저항 확률 하한.',
  outgoingDamageMult: '죽음의 문턱 상태에서 가하는 피해 배율.',
  // 스트레스 (0~10 스케일)
  ...
  deathsDoorStress: '죽음의 문턱 진입 시 스트레스 증가량.',
  allyDownStress: '동료 다운 목격 시 나머지 아군 스트레스 증가량.',
  nightRoundStress: '야간 전투 라운드당 스트레스(광원 없음).',
  nightLitRoundStress: '광원 보유 시 야간 라운드 스트레스.',
  // 전투 토큰 (1회 소비형)
  blockDamageMult: 'block 토큰: 받는 피해 배율(1회 소비).',
  strengthDamageMult: 'strength/power/improvised 토큰: 다음 공격 피해 배율(공격당 1개 소비).',
  vulnerableDamageMult: 'vulnerable 토큰: 받는 피해 배율(1회 소비).',
  hesitationDamageMult: 'hesitation 토큰: 다음 공격 피해 감소 배율(1회 소비).',
  accuracyBonus: 'accuracy 토큰: 다음 공격 명중 보정(1회 소비).',
  focusCritBonus: 'focus 토큰: 다음 공격 치명타 확률 보정(1회 소비).',
  speedInitiativeBonus: 'speed 토큰: 다음 라운드 이니셔티브 굴림 보정(스택당, 라운드 시작 시 소비).',
  markedDamageMult: 'marked(표식) 토큰: 받는 피해 배율(1회 소비) — 집중 공격 시너지.',
  // 랭크 위치 시너지
  ...
  fishingQualityBase: '구의 fishingQuality 기준치. 이 값이면 어획률 보정 0, 미선언 구도 이 값으로 본다.',
  qualityBonusPerStep: 'fishingQuality가 기준치를 1 넘을 때마다 더해지는 어획률.',
  netQtyBonus: '투망 소지 시 낚시 성공 수량 보너스.',
  lureRareBonus: '명인의 루어 소지 시 희귀 대형어 확률 보너스.',
  crabTrapBonus: '게 통발 소지 시 통발 어획률 보너스.',
  autoTrapBaitSave: '자동 포획 장치 소지 시 통발 미끼 소모를 건너뛸 확률.',
  xpPerCast: '낚시 시도 XP.',
...
const ITEM_HELP = {
  ...
  temperature: '체온 변화.',
  trauma: '정신적 트라우마 변화.',
  loneliness: '외로움 수치 변화(음수=감소).',
  tpCost: '이 행동에 드는 시간(TP).',
  ...
  restFatigueMult: '장착 중 휴식 피로 회복 배율 (담요·침낭). 1보다 크면 더 많이 회복한다.',
  ...
  gather: '카드에서 직접 채집하는 규칙 { uses: 총 횟수, tpCost, yields: [{definitionId, qty, weight}] }. weight 비중으로 하나를 뽑고, 횟수를 다 쓰면 카드가 사라진다.',
  uses: '채집 총 횟수. 이 횟수를 다 쓰면 카드가 사라진다.',
  yields: '채집 산출 후보 [{definitionId, qty, weight}]. weight 비중으로 하나를 뽑는다.',
  ammoType: '이 탄약이 들어가는 탄종 id. 무기의 combat.requiresAmmo와 대조한다. 미지정이면 자기 id가 탄종.',
  roundsPerPack: '팩 1장이 채우는 발수. 미지정이면 탄창을 가득 채운다. 희소한 특수 탄약일수록 작게 잡는다.',
  ammoEffect: '장전했을 때 무기에 실리는 보정 { damageBonus, statusInflict, multiTarget, noiseOnUse }. 탄창을 비우면 사라진다.',
  damageBonus: '피해 가산값. 부착물(연마·못 개조)과 장전 탄약이 합산해 더한다.',
  ...
  trapData: '함정 설정 데이터.',
  baitCapacity: '미끼 한 장이 채우는 발동 횟수이자 상한. 이 값이 있으면 미끼 카드를 끌어다 넣을 수 있는 도구가 된다.',
  flight: '비행체 이륙 조건 — fuelDrums(필요 연료 드럼 수)·needsKey(시동 열쇠 필요 여부). 인스턴스의 _fuelDrums/_keyed와 대조해 이륙 가능 여부를 판정한다.',
  escapeVehicle: '탈것 탈출 스펙 — 준비물을 보드에 모으면 카드에 출발 버튼이 열리고, 누르면 endingId 엔딩으로 게임이 끝난다 (EscapeVehicleSystem).',
  endingId: '발동할 엔딩 id(endings.js의 키).',
  requires: '출발 시 보드에서 소모할 준비물 목록 — [{ definitionId, qty }].',
  requiresSubLocation: '이 세부장소(landmarks.js의 sl_* id) 안에서만 출발할 수 있다. 비우면 장소 제한 없음.',
  labelKey: '카드 액션 버튼에 쓸 문구 키(locales.js).',
  confirmKey: '출발 직전 확인 창에 쓸 문구 키(locales.js). 엔딩은 되돌릴 수 없어 반드시 한 번 더 묻는다.',
  fuelDrums: '이륙에 필요한 항공 가솔린 드럼 개수.',
  needsKey: '이륙에 시동 열쇠(helicopter_key)가 필요한지 여부. 직접 제작한 기체는 false로 둘 수 있다.',
  dailyUse: '하루 1회 사용 도구 설정 — anxiety/loneliness/trauma 변화량(음수=감소)과 tpCost. 카드도 내구도도 소모하지 않는 영구 사용. 같은 날 재사용은 인스턴스의 _lastDailyUseDay로 차단된다.',
