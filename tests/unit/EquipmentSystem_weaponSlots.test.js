...
describe('플레이어 무기 슬롯 정책', () => {
  ...
  it('구버전 반대 슬롯 장착은 올바른 슬롯으로 교환하고 탄창을 초기화한다', () => {
    ...
    save.board.middle = Array(27).fill(null);
  ...
  it('보드가 가득 차면 부적합 장비를 pendingLoot으로 보존한다', () => {
    const save = JSON.parse(GameState.serialize());
    const fillerIds = Array.from({ length: 47 }, (_, index) => `filler_${index}`);
    ...
    save.board.middle = fillerIds.slice(0, 27);
    save.board.bottom = fillerIds.slice(27, 47);
    save.pendingLoot = [];
