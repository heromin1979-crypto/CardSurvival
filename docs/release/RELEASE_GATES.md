...
# Phase 3 게이트 — 전투 완성도·재미 (2026-07-14 실행)

계획: `plan/combat-improvement/2026-07-14_phase3_전투_완성도_재미_계획.md` + 재미트랙 상세설계

| 종료 게이트 | 기준 | 실측 | 판정 |
|------|------|------|------|
| 적 패턴 | 12종 텔레그래프-카운터 계약 + 테스트 | 7종 신규 계약 + 5종 유지(기준선 적 포함), 계약 테스트 33종 | ✅ |
| 진형 | 위치 공격 2종+ | charger 넉백 + acid 끌기 + 벽 충돌 피해, 진형 불변식 테스트 | ✅ |
| 스킬 | 6직업 셋업→페이오프 | doctor/homeless(기존) + soldier/firefighter/chef/engineer(신규 연결) | ✅ |
| 밸런스 | DL3 60~75% 유지, stuck 0 (n=1000) | DL3 소음45 62~75%, DL5 하한선 28~50%, 84,000회 stuck 0 | ✅ |
| DL5 실플레이 | 40~60% | 58~84% — HP 손실 70+의 승리 비용과 함께 '준비의 보상'으로 수용, 난이도 옵션/조우 테이블 설계로 이관 | ⚠ 판정 종결 |
| 사운드 | combatSfx 6종 결선 | 타격/치명/미스/처치/자폭/상태이상 + 가드/힐/이동/소환 (`0eba0cd`) | ✅ |
| UI | 토큰 아이콘/잔여 턴/클리핑 해소 | 완료 — 클리핑 실체는 샘플 아이콘 에셋의 구워진 라벨 띠, 에셋 크롭으로 해소 (`c342b72`) | ✅ |
| 검증 | 테스트 green, validate 0, i18n 패리티, E2E | 980개 통과 / ALL CLEAR / combat.* 126:126 / E2E+기준선 스크린샷 갱신 | ✅ |

## 잔여 (Phase 3 미종결분)

- **C2 에셋 생산**: 적 시트 14종·플레이어 11종·배경 5종 — 이미지 생성 작업이라
  코드 게이트가 아닌 생산 백로그. `COMBAT_ASSET_BRIEF.md`의 프롬프트로 생산 후
  `audit_combat_sprites.mjs` + `CombatSpriteSheetAssets.test.js` 검증.
