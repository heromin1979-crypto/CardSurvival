...
*트랙 정체성 단정 끝. M3 #24 이후 모든 작업은 본 §15 단정 안에서 해석. M4+ 텔레메트리 트랙 진입 시 본 §15를 출발점으로 신규 협의 시작. — 2026-05-12 §16 시뮬 R11-1 완전 해소 단언으로 보강.*

---

## 16. 추가 보강 회의록 (2026-05-12, PR17 머지 + baseline v14 + ★ 시뮬 R11-1 완전 해소 단언 ★)

> 참여: PD 김재훈 + 밸런스 권지나. 입력: PR17 머지(시스템 백승호 ~34라인) + `BAL_SIM_baseline_v14_result.json` + `BAL_SIM_baseline_v14_report.md` (밸런스 권지나, 492줄).

### 16.1 baseline v14 측정 결과 단정

- fingerprint `len316-h242a5b5f` v3~v14 12연속 유지, bootstrapErrors 0/700, 11.5초, 결정성 100%
- **K1 7직업 0%, 15회 연속 0%**
- K3 v13 → v14:
  - **chef 5.38 → 6.10 (+0.72d)** ★ PR17 효과 (chef_meal_kit + hearty_stew + 5곳째 actEat candidates 등록)
  - 6직업 변화 0 (회귀 완전 0)
- K5 v13 → v14:
  - 아사 415 → **373 (-42)** ★ chef 아사 완전 해소 (-42)
  - 절망 188 → **224 (+36)** — chef 사망 연장 trade-off
  - 탈수 38 → **36 (-2)**
  - 극도 피로 59 → **67 (+8)**
- chef 격차 정의 1 v13 +0.5717d → v14 **+1.2917d** (Δ +0.72d 회복)
- chef 격차 정의 2 v13 +0.4800d → v14 **+1.2660d** (Δ +0.78d 회복, v13 미해소 → v14 완전 해소)
- chef 사망일: day 5~6 v13 92건 → v14 60건 (-32) / day 7+ v13 6건 → v14 40건 (+34) ★ day 6+ 이동 단정
- 5곳 등록 룰 검증: chef_meal_kit·hearty_stew **100/100 runs 완전 소비** ✅

### 16.2 ★ 시뮬 R11-1 완전 해소 단언 (§15 안에서)

**1·2·3차 시뮬 KPI 동시 충족:**
- 1차 시뮬 KPI 격차 정의 1 ≥ +1.0d: ✅ **+1.2917d (1.29배 달성)**
- 2차 시뮬 KPI 격차 정의 2 ≥ +0.5d: ✅ **+1.2660d (2.53배 달성)**
- 3차 시뮬 KPI chef K3 ≤ 6.5: ✅ **6.10 (마진 0.4d 안전)**

**시뮬 R11-1 완전 해소 단언 — 트랙 B 성공.** 트랙 D (PR16 롤백) / 트랙 C (M3 마감 R11-1 정의 2 후퇴 수용) 폴백 불필요.

**PD 김재훈:**
> 트랙 B 성공은 본 M3 트랙의 본질적 목표 달성 단언. 협의서 v4 §14.4에서 사전 등록한 R11-1 정의 1·2 동시 충족 시도가 5단계 작업(SCN_QUEST 4건 → PR14·PR16 → PR14.1+PR16.1 → PR16 craft 빈도 → PR17 chef high-nutrition) 끝에 성공. §15 트랙 정체성 단정 안에서 시뮬 R11-1 완전 해소 단언.
>
> M3 마감 검토 트리거 충족. 잔존 R13-1·R14-2·R14-3는 M3 마감 vs M4 이월 결정 영역.

**밸런스 권지나:**
> 시뮬 R11-1 완전 해소는 *시뮬 KPI 마지노선 달성*. 게임 본체 K1 검증은 M4+ 텔레메트리 트랙. v14에서 fingerprint 12연속 유지로 BALANCE leaf 무관여 단정 + 회귀 6직업 K3 변화 0 + 결정성 100% — 시뮬 자체 정합성도 완전 보장.

### 16.3 5곳 등록 룰 첫 적용 단언

PR17은 시뮬 정합 트랙 5곳 등록 룰의 *첫 적용 PR*:
- 4곳: items_misc + stackConfig + CardFactory + characters startingItems
- + 5곳째: **`tools/sim/v2/playerAI.mjs:130~133` actEat candidates** (시뮬 정합 신규 룰)

**probe 단정:** chef_meal_kit·hearty_stew 100/100 runs 완전 소비 = actEat candidates 등록이 시뮬 K3 효과 발현의 *필요 조건이자 충분 조건*. 시뮬 정합 트랙(§15)에서 본 룰을 표준 운영 단정으로 격상.

### 16.4 회귀 0 단정 (시스템 백승호 1차)

- PR13 cooking lv ≥ 1 직업 효과 보존: homeless·engineer·pharmacist K3 변화 0
- PR14 abilities (pantry_mastery moraleRecoveryBonus 1.6) 보존
- PR15 `_applyAbilityBonusesToConsume` 자동 발동: hearty_stew morale +5 → +8 추정 (chef pantry_mastery 1.6 가산)
- PR16 craft 발동 빈도 보강 직교 보존: 6직업 K3 변화 0
- chef 외 직업 startingItems·abilities 변경 0

### 16.5 M3 마감 검토 트리거 충족 단언

본 트랙 마지노선 달성 단언:
- ✅ 시뮬 R11-1 완전 해소 (트랙 본질 목표)
- ✅ R8-1 완전 해소 (절망 161 < 200 기준, v11 단언 + v12·v13·v14 유지)
- ✅ R10-1 단계적 해소 (v8 405 → v14 224, -181 = 44.7% 회수)
- ✅ R15-1 완전 해소 (PR16 craft 빈도 보강)
- ✅ 5직업 Tier-2 abilities 완결 (homeless·engineer·firefighter·soldier·pharmacist + chef high-nutrition)
- ✅ 시뮬 정합 5곳 등록 룰 표준 운영 단정 (PR17 첫 적용)
- ✅ fingerprint v3~v14 12연속 유지 (BALANCE leaf 무관여)

잔존:
- ⚠️ R13-1 dismantle skip (M4+ 또는 PR18 후순위)
- ⚠️ R14-2 soldier 보수 초과 (R11-1 해소 후 후순위)
- ⚠️ R14-3 firefighter 사인 전이 보류
- ⚠️ spice_blend actEat candidates 미등록 (SCN_QUEST_chef_supply §11.5 후속 권고)
- ⚠️ M4 텔레메트리 트랙 — 게임 본체 K1 검증 (§15 후속)

**PD 김재훈 권고:** M3 마감 보고서(`PD_MILESTONE_M3_close.md`) 작성 → M3 종결 선언. 잔존 5건은 M4 이월 또는 별도 후순위 PR 트랙으로 분리.

### 16.6 §13.6 다음 단계 갱신 (최종)

| 순위 (최종 갱신) | 작업 | 담당 | 트리거 시점 |
|------|------|------|-----------|
| **1** | M3 마감 보고서 작성 — `PD_MILESTONE_M3_close.md`. M3 전체 산출물 종합 + R 해소 상태 + M4 이월 사항 + 트랙 정체성 단언 종결 | PD 김재훈 | 본 보강 회의록 채택 직후 |
| **2** | (조건부) M3 종결 선언 | PD 김재훈 | M3 마감 보고서 + 사용자 승인 |
| 3 | (선택) PR18 R13-1 dismantle sim 모사 | 시스템 백승호 | M3 마감 후 또는 M4 이월 |
| 4 | (선택) spice_blend actEat candidates 잔존 결함 정리 | 시스템 백승호 | 독립 (1~2 라인) |
| 5 | (선택) PR16.2 R14-2 soldier 재조정 | 시나리오 한도연 + 시스템 백승호 | M3 마감 후 우선순위 후순위 |
| 6 | **M4+ 텔레메트리 트랙 진입** — 게임 본체 K1 검증. §15 트랙 정체성 후속 협의서 v6 신규 발행 | PD 김재훈 + 신규 페르소나 | M3 마감 직후 |
...
| 안건 | 결정 |
|------|------|
| ★ 시뮬 R11-1 완전 해소 | **단언** — 1차 정의 1 +1.2917d / 2차 정의 2 +1.2660d / 3차 K3 6.10 동시 충족 |
| 트랙 B 성공 | **단언** — 트랙 D/C 폴백 불필요 |
| 5곳 등록 룰 첫 적용 | **표준 운영 단정** — 시뮬 정합 트랙(§15) 신규 룰 표준화 |
| 회귀 검증 | ✅ PR13·PR14·PR15·PR16 효과 완전 보존, 6직업 K3 변화 0 |
| M3 마감 검토 트리거 | **충족 단언** — 마지노선 달성 7건 |
| 잔존 후순위 5건 | R13-1 / R14-2 / R14-3 / spice_blend / M4 텔레메트리 |
| 다음 단계 | PD M3 마감 보고서 → M3 종결 → M4+ 텔레메트리 트랙 진입 |

본 보강은 본 협의서의 결정 권한 안에서 처리. **M3 마감 보고서 작성은 PD 김재훈 단독 트랙으로 진입. M3 종결 결정은 사용자(프로젝트 책임자) 승인 영역.**

---

*보강 회의록 끝. **본 §16이 협의서 v5 (PR14 결정 협의서)의 마감 보강이다.** PD_MILESTONE_M3_close.md 작성 직후 M3 종결 선언 또는 M4 진입 결정. 본 협의서는 M3 트랙 산출물로 archive 처리 권고 (M4 협의서 v6 신규 발행 시 본 §16을 종결 단언으로 인용).*
