...
**범위 밖:** 나머지 숨겨진 장소 24곳의 세부장소 데이터 작성, 컷씬 배경 이미지 제작, 발견 목록 조회 UI
...
`dangerMod: 0.15`는 같은 랜드마크의 관리 창고(0.10)보다 높고, 프로젝트의 `mid` 임계(0.20) 미만이다. 군 시설이지만 현충원 전체가 "감염자가 기피하는 조용한 구역"이라는 설정과 충돌하지 않는 값이다.

**상단 슬롯 여유:** 현충원 세부장소 3개 + 귀환 카드 1개 = 4칸. 벙커 추가 시 5칸으로, 장소 행 10칸 이내다.
...
- 나머지 숨겨진 장소 24곳의 세부장소 데이터 작성 — 구는 정해져 있으나 소속 랜드마크가 미지정인 곳이 많아 설정 작업이 선행되어야 함
- 발견 컷씬 배경 이미지 제작
