...
**범위**: `css/cards.css`, `css/board.css`, `js/ui/CardFactory.js`
2026-09-07 UI 라운드가 끝난 뒤 목표 이미지와 대조하다 남은 **유일한 UI 차이**다.

### D-1. 카드 안이 빡빡하다

목표(`reference/ui-refs/main-screen-v2.png`)의 장소 카드는 썸네일이 크고 설명이 짧다.
현재는 133px 폭 안에 요소가 다섯이다 — **썸네일 · 이름 · 설명 2줄 · 모서리 배지 3개 ·
위험도 · 요구 TP**(`CardFactory.js:1060` 의 `lc-req`, `1065-1066` 의 `lc-badge`).
그 결과 썸네일이 눌리고 텍스트가 테두리에 붙는다.

**이건 간격 문제가 아니다.** 카드 사이 간격은 이미 고쳤다 —
`--card-w` 를 두 곳에서 정의하던 것을 맞춰 겹침 -8.6px → +18.4px 이 됐다 (8a9b6d9).
남은 것은 **카드 안쪽 여백**이다. 밖은 벌어졌는데 안이 좁다.

고칠 방향 후보 (하나를 고르고 이유를 STATUS 에 남긴다):
- 카드 안쪽 `padding` 을 키우고 설명을 2줄 → 1줄로 줄인다
- 배지 3개 중 덜 중요한 것을 hover 로 내린다
- 위험도·요구 TP 를 한 줄로 합친다

**측정으로 판정한다.** `tools/measure-card-overlap.mjs` 가 카드/슬롯 좌표를 재고,
`tools/capture-main-screen.mjs` 가 1920×1080 캡처를 남긴다. 캡처를 목표 이미지와
나란히 놓고 썸네일 비율을 비교하는 것이 합격 판정이다.

### D-2. 바닥 행 아이콘 스타일이 다르다 (에셋 작업)

목표는 아이템 아이콘이 크고 배경이 투명한데, 현재는 사진 썸네일이 카드를 가득 채운다.
**CSS 로 해결되지 않는다** — 에셋 자체를 바꿔야 한다. UI 라운드 밖이다.
여기 적어 두는 것은 목표와의 차이를 빠뜨리지 않기 위해서다.

### D-3. 범위 밖으로 남겨 둔 둘 (재확인)

2026-09-07 라운드에서 SPEC 4절로 제외한 항목이고 여전히 유효하다.

- **배경 사진 레이어** — 목표는 폐허 사진이 전면에 깔린다. `assets/images/` 에 에셋이 없다
- **한/영 병기** — `nameEn` 필드가 저장소 어디에도 없다 (아이템 659종 전부).
  아이템 659 + 랜드마크 45개에 영문명을 새로 채워야 하는 별도 프로젝트다
