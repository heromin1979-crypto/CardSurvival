# STATUS — 어디까지 했고 다음은 뭔가

> 한 바퀴마다 갱신한다. 루프의 유일한 기억이다.
> "다음에 할 것"이 비면 다음 바퀴는 이어받을 자리를 모른다. 반드시 채운다.

## 지금 상태

<!-- 한 문단. 현재 어디까지 동작하는가. -->

메인 화면은 아직 2컬럼(좌 사이드바 200px + 보드)이고 `#game-header`(56px)는 비어 있다.
0군(가리는 것)은 끝났다 — 첫 드래그 안내 오버레이가 헤더 띠로 올라가 보드 카드를 덮지 않는다.
1군(우측 동료 패널 · 상단 HUD 칩 · 사이드바 재편)부터는 아직 손대지 않았다.
캡처 확인 결과 바닥 행 10칸이 모두 보이고 `npm test` 3381건 · `node js/data/validate.js` Errors 0.

## 끝난 것

<!-- 최신이 위. 형식: - [YYYY-MM-DD] 무엇을 / 커밋해시 -->

- [2026-09-06] 온보딩 첫 드래그 안내를 화면 정중앙 → 상단 헤더 띠로 이동, `✕` 닫기 버튼 추가,
  재중앙화 회귀 검사(`tests/unit/OnboardingBoardTooltip.test.js`) 추가 / d59957e

## 다음에 할 것

<!-- 하나만. 가장 위가 다음 바퀴가 집을 작업이다. -->
<!-- 형식: - [ ] 무엇을 / 어디 파일 / 무엇을 보면 됐다고 판단하는가 -->

- [ ] INBOX 1군 첫 항목 — 우측 동료 패널 컬럼(폭 약 180px)을 만든다 /
  `css/layout.css`의 `#screen-main.active` 그리드(`200px 1fr` → 동료 컬럼 추가) +
  `js/screens/Basecamp.js`(사이드바·메인 조립부) + `js/ui/` 컴포넌트 /
  캡처에 우측 패널이 보이고 보드 폭이 줄어든 만큼 장소 10칸·휴대 10열이 깨지지 않으면 됐다.

## 결정과 근거

<!-- 왜 그렇게 정했는지. 이게 남아야 다음 바퀴가 되돌리지 않는다. -->
<!-- 형식: - [YYYY-MM-DD] 무엇을 이렇게 정했다 / 왜 -->

- [2026-09-06] 온보딩 안내를 **상단 헤더 띠의 왼쪽(사이드바 오른쪽)** 에 뒀다 /
  1920×1080에서 보드가 화면을 거의 다 채워 "보드를 덮지 않는 자리"는 비어 있던 헤더 띠뿐이었다.
  가운데를 비워 둔 이유는 다음 바퀴가 거기에 HUD 칩(`Day | 시각 | 온도`)을 넣기 때문이고,
  오른쪽은 알림 패널(`#notification-container`, top/right 16px) 자리다.
  HUD 칩 작업 때 `game-header__left`(위치 브레드크럼)와 겹치면 그때 다시 옮긴다.
- [2026-09-06] 툴팁을 세로 카드에서 **한 줄 가로 칩**으로 눕혔다 /
  헤더 띠 높이가 56px이라 기존 3단 세로 카드(약 110px)는 들어가지 않는다. 문구는 그대로 두었다.
- [2026-09-06] 닫기 버튼을 넣되 별도 핸들러를 달지 않았다 /
  기존 `overlay.addEventListener('click', dismiss)` 로 버블링돼 같은 경로로 닫힌다.
  경로가 둘이면 한쪽만 고쳐질 수 있다.
- [2026-09-06] 좌표 검사 대신 **CSS 선언 + DOM 검사**로 회귀를 막았다 /
  테스트 환경(happy-dom)은 레이아웃을 계산하지 않아 "카드를 덮는지"를 좌표로 잴 수 없다.
  대신 `#onboarding-board-tooltip` 블록이 다시 `align-items/justify-content: center` 로
  돌아가는지와 닫기 버튼 존재·동작을 검사한다. 되돌리면 빨간불이 뜨는 것은 확인했다.

## 막힌 것

<!-- 사람이 풀어줘야 하는 것. 루프는 여기를 건드리지 말고 넘어간다. -->

- [2026-09-06] **Confluence 패치 노트를 못 남겼다.** `CLAUDE.md` 7절이 게임 동작이 바뀌는
  커밋마다 패치 노트를 요구하는데, 이 세션에서 Atlassian MCP 도구 권한이 없어
  `createConfluencePage` 호출이 거부됐다 (d59957e 분). 사람이 권한을 열어 주거나
  대신 페이지를 만들어야 한다. 다음 바퀴도 같은 벽에 부딪힌다.
