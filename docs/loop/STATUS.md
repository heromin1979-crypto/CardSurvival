...
- [ ] INBOX 1군 첫 항목 — 우측 `동료 상태` 블록(사기 MORALE / 유대 BOND)을 만든다 /
  `js/ui/CompanionPanel.js` + `css/companion-panel.css`. 1바퀴에서 만든 `동료` 패널
  아래에 두 번째 블록으로 붙인다 /
  캡처에 두 블록이 보이고 180px 폭 안에서 넘치지 않으면 됐다.
  ※ 사기는 플레이어 전역(`GameState.js:104`), 유대는 동료별(`NPCSystem` bond).
     범위가 다르다 — INBOX 항목의 설명을 반드시 읽어라.
  ※ 그 다음은 상단 중앙 HUD 칩이다. 온보딩 칩이 헤더 왼쪽을 쓰니 그 자리를 뺏지 마라
     (아래 "결정과 근거" 참조).
